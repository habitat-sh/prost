...
fn bench_encode_message<M>(b: &mut test::Bencher, message: &M) where M: Message  {
...
fn bench_decode_message<M>(b: &mut test::Bencher, bytes: &[u8]) where M: Message + Default {
    b.iter(|| {
...
fn google_message1_encode(b: &mut test::Bencher) {
    ...
    bench_encode_message(b, &message);
}

#[bench]
fn google_message1_decode(b: &mut test::Bencher) {
    bench_decode_message::<messages::GoogleMessage1>(b, GOOGLE_MESSAGE1);
}
